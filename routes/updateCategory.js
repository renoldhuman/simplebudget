...
router.post('/', function(req, res, next) {
  ...
  var catAlloc = req.body.newCatAlloc * 1;

  console.log(uName + " " + catName + " " + catAlloc);

  var newCatJSON = {$set : {categories: {categoryName: catName, allocation: catAlloc}}};
  ...
  MongoClient.connect('mongodb+srv://thelmrich:mAngodatabas333@simplebudget-cluster-cn9l5.mongodb.net/SimpleBudget?retryWrites=true', function(err, client){
    ...
    db.collection('users').updateOne(
      {username: uName, "categories.categoryName": catName},
      {$set: {"categories.$.allocation": catAlloc}},
      function(err, res) {
        if (err) throw err;
